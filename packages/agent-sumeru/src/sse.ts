...
/**
 * Stateful incremental SSE parser. Feed bytes via `push`; consume completed
 * events from the returned array. Trailing partial event (no blank-line
 * terminator) is retained internally and emitted on the next `push` that
 * completes it.
 */
